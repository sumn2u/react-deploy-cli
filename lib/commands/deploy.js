...
module.exports = {
  ...
  availableOptions: [
    { name: 'deploy-config-file', type: String, description: '(Default: deploy.js)' },
  ...
  run: function(commandOptions, rawArgs) {
    ...
    // commandOptions.deployTarget = (rawArgs == undefined) ? 'production': rawArgs.shift()
    console.log('rawArgs', rawArgs, "command options")
    commandOptions.deployTarget = rawArgs.shift() | 'production';
    commandOptions.deployConfigFile = chooseOptionValue(commandOptions.deployConfigFile, this.settings, 'deploy-config-file', 'deploy.js');
